...
class UnitLessonTest extends TestCase
{
    ...
    public function test_can_delete_unit(): void
    {
        ...
        $this->withToken($this->token)
            ->deleteJson("/api/v1/units/{$unit->unit_id}")
            ->assertOk();
    }

    public function test_can_reorder_units(): void
    {
        $unit2 = Unit::create([
            'course_id'               => $this->course->course_id,
            'title'                   => ['en' => 'Unit 2'],
        ...
        $this->withToken($this->token)
            ->postJson("/api/v1/courses/{$this->course->slug}/units/reorder", [
                'unit_orders' => [
                    ['unit_id' => $this->unit->unit_id, 'unit_order' => 2],
                    ['unit_id' => $unit2->unit_id,       'unit_order' => 1],
                ],
            ])
            ->assertOk();
    }
